...
async (req, res) => {
    try {
        if (!req.donor || req.donor.role !== 'manager') {
            res.status(403).json({ error: 'Access denied' })
        }
        const { name, username, email, phone, password } = req.body
        if (!username && !email && !password)
            return res.status(400).json({ messange: 'username & email & password is must files' })
        const donor = await Donor.create({ username: username, name: name, email: email, phone: phone, password: password })
        res.json(donor)
    } catch (error) {
...
async (req, res) => {
    try {
        ...
        const {donorId, name, username, email, phone, password } = req.body
        const donor = await Donor.findOne({ donorId }).exec()
        if (!donor) {
            return res.status(404).json({ message: "Donor not found" })
        }
